var AppProcess = (function () {
    ...
    function eventProcess() {
        $("#miceMuteUnmute").on("click", async function () {
            ...
            if (isAudioMute) {
                ...
                await updateMediaSenders(audio, rtp_aud_senders);
            } else {
                audio.enabled = false;
                $(this).html("<span class='material-icons' style='width:100%'>mic-off</span>")
                await updateMediaSenders(rtp_aud_senders);
            }
    ...
    function removeMediaSenders(rtp_senders) {
        for (var con_in in peers_connection_ids) {
    ...
    function removeVideoStrem(rtp_vid_senders) {

        if (videoCamTrack) {
            videoCamTrack.stop();
            videoCamTrack = null;
            local_div.srcOject = null;
    ...
    async function videoProcess(newVideoState) {
        if (newVideoState == video_states.None) {
            ...
            removeVideoStrem(rtp_vid_senders)
            return;
        }
        ...
        try {
            ...
            if (vstream && vstream.getVideoTracks().length > 0) {
                ...
                if (videoCamTrack) {
                    local_div.srcOject = new MediaStream([videoCamTrack]);
